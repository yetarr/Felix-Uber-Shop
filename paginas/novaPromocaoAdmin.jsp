            ...
            try {
                ...
                if (newId > 0 && prodIds != null) {
                    ...
                    for (String pid : prodIds) {
                        ps.setInt(1, newId); ps.setInt(2, Integer.parseInt(pid));
                        ps.executeUpdate();
                    }
                    ps.close();
                }
    ...
    try {
        ...
        while (_rs.next()) {
            catalogue.add(new Object[]{
                String.valueOf(_rs.getInt("id_produto")),
                _rs.getString("nome"),
                (int) _rs.getLong("preco_cents")
            });
        }
    } catch (Exception _e) {
    } finally {
        closeAll(_rs, _ps, _conn);
    }
%>
<!DOCTYPE html>
<html lang="pt">
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #212121; color: #e0e0e0; font-family: 'Segoe UI', Arial, sans-serif; min-height: 100vh; display: flex; flex-direction: column; }

        .topnav { background: #2a2a2a; border-bottom: 1px solid #333; height: 52px; display: flex; align-items: center; justify-content: space-between; padding: 0 24px; position: sticky; top: 0; z-index: 200; }
        ...
        .btn-sair { background: none; border: 1px solid #555; color: #aaa; font-size: .8rem; padding: 5px 14px; border-radius: 6px; cursor: pointer; text-decoration: none; transition: border-color .2s, color .2s; }
        .btn-sair:hover { border-color: #e05555; color: #e05555; }

        .app-shell { display: flex; flex: 1; min-height: 0; }

        .sidebar { width: 215px; flex-shrink: 0; background: #111; border-right: 1px solid #1e1e1e; display: flex; flex-direction: column; padding: 24px 0 16px; }
        .sidebar-label { font-size: .67rem; font-weight: 700; letter-spacing: 1.2px; color: #3a3a3a; text-transform: uppercase; padding: 0 20px 14px; }
        .sidebar-nav { list-style: none; flex: 1; }
        .sidebar-nav li a { display: flex; align-items: center; gap: 11px; padding: 11px 20px; text-decoration: none; font-size: .88rem; color: #777; border-left: 3px solid transparent; transition: background .15s, color .15s, border-color .15s; }
        .sidebar-nav li a:hover { background: rgba(255,255,255,.04); color: #ddd; }
        .sidebar-nav li a.active { border-left-color: #00CE86; background: rgba(0,206,134,.07); color: #00CE86; font-weight: 600; }
        ...
        .panel-header { padding: 14px 18px; border-bottom: 1px solid #333; display: flex; align-items: center; gap: 9px; }
        .panel-title { font-size: .95rem; font-weight: 700; color: #e8e8e8; }
        .panel-title svg { fill: #00CE86; width: 17px; height: 17px; }

        .alert { margin: 14px 18px 0; padding: 10px 14px; border-radius: 8px; font-size: .84rem; display: flex; align-items: center; gap: 8px; }
        ...
        .dates-row { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }

        .prods-label { font-size: .78rem; color: #888; margin-bottom: 6px; display: block; }
        .prod-list { border: 1px solid #2e2e2e; border-radius: 8px; overflow: hidden; margin-bottom: 18px; }
        .prod-item { display: flex; align-items: center; justify-content: space-between; padding: 9px 12px; border-bottom: 1px solid #242424; cursor: pointer; transition: background .15s; }
        .prod-item:last-child { border-bottom: none; }
        .prod-item:hover { background: rgba(255,255,255,.03); }
        .prod-item-left { display: flex; align-items: center; gap: 9px; }
        .prod-item input[type="checkbox"] { accent-color: #00CE86; width: 15px; height: 15px; cursor: pointer; flex-shrink: 0; }
        .prod-item-nome { font-size: .86rem; color: #ccc; }
        .prod-item-preco { font-size: .83rem; color: #666; }
        .prod-item-disc { font-size: .83rem; color: #00CE86; font-weight: 600; }

        .btn-criar { width: 100%; background: #00CE86; color: #111; border: none; border-radius: 8px; padding: 11px; font-size: .92rem; font-weight: 700; cursor: pointer; transition: background .2s; margin-bottom: 10px; }
        .btn-criar:hover { background: #00b876; }
        .btn-voltar { display: block; text-align: center; color: #666; font-size: .84rem; text-decoration: none; padding: 8px; transition: color .2s; margin-bottom: 14px; }
        .btn-voltar:hover { color: #aaa; }

        /* Preview panel */
        .preview-panel { background: #262626; border: 1px solid #333; border-radius: 10px; overflow: hidden; }
        .preview-header { padding: 12px 16px; border-bottom: 1px solid #333; font-size: .72rem; font-weight: 700; letter-spacing: .8px; text-transform: uppercase; color: #555; }
        .promo-card { margin: 14px 16px 10px; background: #00CE86; color: #111; border-radius: 8px; padding: 12px 14px; }
        .promo-card-title { font-size: .9rem; font-weight: 700; line-height: 1.3; }
        .promo-card-dates { font-size: .73rem; margin-top: 4px; opacity: .75; }
        .preview-prods-label { font-size: .75rem; color: #888; padding: 10px 16px 6px; }
        .preview-prod-item { display: flex; justify-content: space-between; align-items: center; padding: 6px 16px; border-top: 1px solid #2a2a2a; }
        .preview-prod-nome { font-size: .83rem; color: #ccc; }
        .preview-prod-prices { display: flex; align-items: center; gap: 6px; }
        .price-orig { font-size: .78rem; color: #555; text-decoration: line-through; }
        .price-disc { font-size: .83rem; color: #00CE86; font-weight: 700; }
        .preview-empty { padding: 16px; font-size: .82rem; color: #555; font-style: italic; text-align: center; }
    ...
    <aside class="sidebar">
        ...
        <ul class="sidebar-nav">
            ...
            <li><a href="encomendasAdmin.jsp">
                <svg viewBox="0 0 24 24"><path d="M20 6h-2.18A3 3 0 0 0 15 4H9a3 3 0 0 0-2.82 2H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-11 0h6c.55 0 1 .45 1 1s-.45 1-1 1H9c-.55 0-1-.45-1-1s.45-1 1-1z"/></svg>
