...
<%
    ...
    try {
        ...
        ps = conn.prepareStatement("SELECT COUNT(*) FROM auditoria_carteira");
        rs = ps.executeQuery();
        if (rs.next()) statCarteira = rs.getInt(1);
        closeAll(rs, ps, null);

        ps = conn.prepareStatement("SELECT COUNT(*) FROM auditoria WHERE categoria='Utilizador'");
        rs = ps.executeQuery();
        if (rs.next()) statUtilizador = rs.getInt(1);
        closeAll(rs, ps, null);

        ps = conn.prepareStatement("SELECT COUNT(*) FROM auditoria WHERE categoria='Produto'");
        rs = ps.executeQuery();
        if (rs.next()) statProduto = rs.getInt(1);
        closeAll(rs, ps, null);

        ps = conn.prepareStatement("SELECT COUNT(*) FROM auditoria WHERE categoria='Encomenda'");
        rs = ps.executeQuery();
        if (rs.next()) statEncomenda = rs.getInt(1);
        closeAll(rs, ps, null);

        ps = conn.prepareStatement("SELECT COUNT(*) FROM auditoria WHERE categoria='Promoção'");
        rs = ps.executeQuery();
        if (rs.next()) statPromocao = rs.getInt(1);
        closeAll(rs, ps, null);
        ...
        while (rs.next()) {
            logs.add(new String[]{
                    String.valueOf(rs.getTimestamp("data_evento")),
...
<div class="app-shell">
    ...
    <main class="main-content">
        ...
        <div class="stats-row">
            <div class="stat-card">
                <div class="stat-num color-carteira"><%= statCarteira %>
                </div>
                <div class="stat-label">Carteira</div>
            </div>
            <div class="stat-card">
                <div class="stat-num color-utilizador"><%= statUtilizador %>
                </div>
                <div class="stat-label">Utilizadores</div>
            </div>
            <div class="stat-card">
                <div class="stat-num color-produto"><%= statProduto %>
                </div>
                <div class="stat-label">Produtos</div>
            </div>
            <div class="stat-card">
                <div class="stat-num color-encomenda"><%= statEncomenda %>
                </div>
                <div class="stat-label">Encomendas</div>
            </div>
            <div class="stat-card">
                <div class="stat-num color-promocao"><%= statPromocao %>
                </div>
        ...
        <div class="audit-panel">
            ...
            <div class="filter-bar">
                ...
                <input type="date" id="filterAte" class="filter-date" value="<%= filterAte %>"
            ...
            <table class="audit-table" id="auditTable">
                ...
                <tr data-cat="<%= lCat %>"
                    ...
                    data-valor="<%= valorNum %>">
                    <td class="col-data"><%= lData %>
                    </td>
                    <td><span class="cat-badge <%= badgeClass %>"><%= lCat %></span></td>
                    <td class="col-acao"><%= lAcao %>
                    </td>
                    <td class="col-user"><%= lUser %>
                    </td>
                    <td class="col-detalhe"><%= lDetalhe %>
                    </td>
                    <td class="<%= valorClass %>"><%= valorDisplay %>
                    </td>
                </tr>
                <% } %>
                </tbody>
