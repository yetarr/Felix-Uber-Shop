...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        if ("guardar".equals(postAction)) {
            ...
            if (titulo == null || titulo.isBlank() || descontoStr == null || dataInicio == null || dataFim == null) {
                ...
                try {
                    ...
                    if (promoId == null || promoId.isBlank()) {
                        ...
                        savedId = keys.next() ? keys.getInt(1) : -1;
                        keys.close();
                        ps.close();
                    } else {
                    ...
                    response.sendRedirect("promocoesAdmin.jsp?promoId=" + savedId);
                    return;
            ...
            try {
                ...
                response.sendRedirect("promocoesAdmin.jsp?promoId=" + pid);
                return;
            } catch (Exception e) {
                errorMsg = "Erro: " + e.getMessage();
            }
    ...
    try {
        ...
        while (rs2.next()) {
            promos.add(new Object[]{
                    String.valueOf(rs2.getInt("id_promocao")),
                    rs2.getString("titulo"),
                    rs2.getBigDecimal("desconto_percentagem").intValue(),
                    rs2.getString("data_inicio"),
                    rs2.getString("data_fim"),
                    rs2.getInt("ativo") != 0
            });
        }
        ...
        while (rs2.next()) {
            catalogue.add(new Object[]{
                    String.valueOf(rs2.getInt("id_produto")),
                    rs2.getString("nome"),
                    (int) rs2.getLong("preco_cents")
            });
        }
    } catch (Exception _e2) {
        // Listas ficam vazias em caso de erro
    } finally {
        closeAll(rs2, ps2, conn2);
    }
    ...
    if (!selectedId.isEmpty()) {
        ...
        try {
            ...
            ps3.setInt(1, Integer.parseInt(selectedId));
            rs3 = ps3.executeQuery();
            while (rs3.next()) {
                selProds.add(String.valueOf(rs3.getInt("id_produto")));
            }
        } catch (Exception _e2b) {
            // Lista de produtos fica vazia em caso de erro
        } finally {
            closeAll(rs3, ps3, conn3);
        }
    }
%>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        *, *::before, *::after {
        ...
        .promos-table tr:last-child td {
            border-bottom: none;
        }

        .promos-table tr.selected td {
        ...
        @media (max-width: 580px) {
            ...
            .promos-table th:nth-child(4), .promos-table td:nth-child(4) {
                display: none;
            }
        }
    </style>
            ...
            <div class="panel">
                ...
                <table class="promos-table" id="promosTable">
                    ...
                    <tr class="<%= isSel ? "selected" : "" %>"
                        ...
                        <td>—</td>
                        <td><span
                                class="badge <%= pativo ? "badge-ativo" : "badge-inativo" %>"><%= pativo ? "Ativo" : "Inativo" %></span>
                        </td>
    ...
    function selectPromo(id) {
        ...
        if (p.ativo) {
            ...
            btn.onclick = function () {
                return confirm('Inativar ' + p.titulo + '?');
            };
        } else {
            btn.textContent = 'Ativar promoção';
            btn.classList.add('ativar');
            btn.onclick = function () {
                return confirm('Ativar ' + p.titulo + '?');
            };
        }
